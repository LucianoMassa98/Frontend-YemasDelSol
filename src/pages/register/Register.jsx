...
import { Button, TextField, Box, Collapse, Alert, IconButton, InputAdornment } from '@mui/material';
...
export default function Register() {
    const [probandoError, setProbandoError] = useState(false)
    const [isFormValid, setIsFormValid] = useState(false);
    const [previousHasErrors, setPreviousHasErrors] = useState(false);
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.celular.trim() === "") {
            ...
            if (formData.celular.trim().length > 13 || formData.celular.trim().length < 3) {
                setAllErrors(prevErrors => ({
                    ...prevErrors,
                    calular: true,
        ...
        if (formData.usuario.trim() === "") {
            ...
            if (formData.usuario.trim().length > 15 || formData.usuario.trim().length < 4) {
                setAllErrors(prevErrors => ({
                    ...prevErrors,
                    usuario: true
                }));
        ...
        if (formData.contraseña.trim() === "") {
            ...
            if (formData.contraseña.trim().length < 8 || !/\d/.test(formData.contraseña.trim())) {
                setAllErrors(prevErrors => ({
                    ...prevErrors,
                    contraseña: true
                }));
                setTextError("La contraseña debe tener más de 8 caracteres y contener al menos un número");
            }
            else {
                setAllErrors(prevErrors => ({
                    ...prevErrors,
        ...
        if (formData.nombre !== "" && formData.apellido !== "" && formData.celular !== "" && formData.email !== "" && formData.contraseña !== "" && formData.confirmarContraseña !== "" && formData.usuario !== "") {
            setErrors(prevErrors => {
            ...
            if (hasErrors) {
                ...
                const updatedHasErrors = Object.values(allErrors).some(error => error);
            ...
            >
                ...
                <div className='cont'>
                    ...
                    <TextField
                        ...
                        helperText={errors.confirmarContraseña ? "este campo es requerido" : ""}
                        variant="standard"
                        fullWidth
                        InputProps={{
                            endAdornment: (
                    ...
                    <Collapse in={allErrors.nombre || allErrors.apellido || allErrors.email || allErrors.contraseña || allErrors.celular || allErrors.usuario || allErrors.confirmarContraseña}>
